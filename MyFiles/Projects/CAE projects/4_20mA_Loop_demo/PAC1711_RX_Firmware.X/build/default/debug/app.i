...
# 50 "./mcc_generated_files/system/../system/../system/clock.h"
...
# 46 "./mcc_generated_files/system/../uart/uart1.h"
# 1 "./mcc_generated_files/system/system.h" 1
...
void APP_Tasks ( void )
{
    ...
    switch ( appData.state ){
        ...
        case APP_STATE_INIT:
        {
            if(PAC1711_Device_IsInitialized(&deviceContext) == 1){
                appData.state = APP_STATE_DEVICE_ID;
            }
            break;
        }
        ...
        case APP_STATE_REFRESH:
        {
            ...
            if (errorCode == 0 && !I2C1_IsBusy()){

                printf("Refresh done");
                DELAY_milliseconds(200);
                appData.state = APP_STATE_READ_VBUS;
            }else{
                printf("fail: %d", errorCode);
            }
            break;
        ...
        case APP_STATE_READ_VBUS:
        {
            ...
            errorCode = PAC1711_GetVSENSE_mV(&deviceContext, &voltage);
            if (errorCode == 0 && !I2C1_IsBusy()){

                printf("%f", voltage);
                 DELAY_milliseconds(50);
                appData.state = APP_STATE_READ_VSENSE;
            }else{
                printf("fail: %d", errorCode);
            }
            break;
        ...
        case APP_STATE_READ_POWER:
        {
            ...
            if (errorCode == 0){
                printf("%f", power);
                DELAY_milliseconds(2);
                appData.state = APP_STATE_READ_ENERGY;
            }else{
                printf("fail: %d", errorCode);
            }
            break;
        ...
        case APP_STATE_WAIT:{
            if(appData.wait_started == 0){

                appData.wait_MS = 2000;
                appData.wait_started = 1;
